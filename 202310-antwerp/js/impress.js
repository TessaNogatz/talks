...
( function( document ) {
    ...
    document.addEventListener( "impress:init", function( event ) {
        ...
        gc.addEventListener( document, 'impress:blackout:add', function (event) {
            blackout();
        })

        gc.addEventListener( document, 'impress:blackout:remove', function (event) {
            removeBlackout();
        })

        gc.addEventListener( document, 'impress:blackout:switch', function (event) {
            if ( !blackedOut ) {
                blackout();
            } else {
                removeBlackout();
...
( function( document, window ) {
    ...
    var impressConsole = window.impressConsole = function( rootId ) {
        ...
        var blackout = function() {
            var slideView = consoleWindow.document.getElementById( 'slideView' );
            triggerEvent( document, 'impress:blackout:switch' );
            triggerEventInView(slideView, 'impress:blackout:switch' );
        }
        ...
        var consoleOnLoad = function() {
                ...
                slideView.contentWindow.addEventListener('hashchange', function (event) {
                    if (event.newURL !== event.oldURL) {
                        impress().goto(decodeURIComponent(new URL(event.newURL).hash.replace( /^#\/?/, "" )));
                    }
                })
        };
        ...
        var open = function() {
            ...
            if ( consoleWindow && !consoleWindow.closed ) {
                ...
                // 66: B
                registerKeyEvent( [ 66 ], blackout );

                // 79: O
                registerKeyEvent( [ 79 ], gotoOverview );
...
( function( document, window ) {
    ...
    var show = function() {
        ...
        window.timeoutHandle = window.setTimeout( hide, timeout * 1000 );
    };
...
( function( document ) {
    ...
    var selectOptionsHtml = function() {
        ...
        for ( var i = 0; i < steps.length; i++ ) {
            ...
            if ( hideSteps.indexOf( steps[ i ] ) < 0 && !steps[i].classList.contains('skip-progress')) {
                if (steps[i].classList.contains('backup')) {
                    backupCount++;
                } else {
                    slideCount++;
                }
                options = options + '<option value="' + steps[ i ].id + '">' + // jshint ignore:line
                            (
                                `${steps[i].classList.contains('backup') ? `B${backupCount.toString().padStart(2, '0')}` : slideCount.toString().padStart(2, '0')}: ${getStepTitle( steps[ i ], slideCount )}`
                            ) + '</option>' + '\n';
            }
        }
        return options;
...
( function( document ) {
    ...
    function updateProgressbar( slideId ) {
        ...
        var currentSlide;
        var backupsCount = 0;
        var currentSlideBackupCount = 0;
        for (var i = 0; i < steps.length; i++) {
            if (!steps[i].classList.contains('skip-progress') && !steps[i].classList.contains('backup')) {
                slideCount++;
            }
            if (steps[i].classList.contains('backup')) {
                backupsCount++;
            }
            if (!slideNumber && steps[i].id === slideId) {
                slideNumber = slideCount;
                currentSlide = steps[i];
                currentSlideBackupCount = backupsCount;
            }
        }
        if (currentSlide.dataset.progressText) {
            progress.innerHTML = currentSlide.dataset.progressText;
            return
        }
        if (currentSlide.dataset.progress) {
            slideNumber = parseInt(currentSlide.dataset.progress)
        }
        if ( null !== progressbar ) {
            var width = 100 / slideCount * ( slideNumber );
            progressbar.style.width = width.toFixed( 2 ) + "%";
        }
        if ( null !== progress ) {
            if (currentSlide.classList.contains('backup')) {
                progress.innerHTML = `Backup-${currentSlideBackupCount}`;
            } else {
                if (slideNumber == 0) {
                    progress.innerHTML = slideCount;
                } else {
                    progress.innerHTML = slideNumber + " / " + slideCount;
                }
            }
        }
    }
...
( function( document, window ) {
    ...
    var hideSubstep = function( visible ) {
        if ( visible.length > 0 ) {
            ...
            if (current > 0 && visible[current-1].dataset.substepOrder === visible[current].dataset.substepOrder) {
                visible.pop();
